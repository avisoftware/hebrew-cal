    ...
    if(!init()){
       ...
       LocationLayout->addWidget(labelLocation);
        ...
        LocationLayout->addStretch(1);
         ...
         considerHeightCheckBox = new QCheckBox(QString::fromLocal8Bit("התחשבות בגובה מיקום - משנה את זמני השקיעה/זריחה"));
       ...
       winterRadioButton = new QRadioButton(QString::fromLocal8Bit("שעון חורף"),summerTimeBox);
        ...
        if (f=="h:mm:ss") {
            ...
            formatTimeComboBox->setCurrentIndex(4);
        }
         else if(f=="hh:mm ap"){
            formatTimeComboBox->setCurrentIndex(5);
        }else{
              formatTimeComboBox->setCurrentIndex(2);
        }
