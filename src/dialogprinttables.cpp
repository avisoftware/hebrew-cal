...
void DialogPrintTables::init (int m,int y){
        ...
        for(int i=5750;i<5800;i++){
            yearL.append(hdate_get_int_string(i));
        }
   ...
   for(int i=step;i>l;i--){
       if(p<LMonth.count()){
           ...
           html= html + (new_table+"<td "+width+" class='month'><div dir='rtl'> "
                         ...
                         " <tr  align=center>"
                         " <td colspan='8' align='right' class='head'> " +hdate_get_hebrew_month_string(h.hd_mon,0)+" "+hdate_get_int_string(year)+ "</td>	"
                         "</tr>"
                          "<tr>"
           ...
           for(int j= 1;j<31;j++){
               ...
               html= html +" <tr class='day_detail'>"
                       ...
                       " <td align=center class='h_day_t'> " +hdate_get_int_string(h.hd_day)+ " </td>"
                       "<td align=center class='day_in_w' > " +hdate_get_day_in_week_string(h.hd_dw,0)+ "</td>"
     ...
     for(int i=0;i<LMonth.count();i++){
             ...
             if (h.gd_year==h_ent_m.gd_year){
                ...
                html= html + (" <tr  align=right>"
                           " <td colspan='7' align='center' class='head'> "+hdate_get_hebrew_month_string(h.hd_mon,0)+" "+hdate_get_int_string(year)+"</td>	"
                           "</tr>"
                            "<tr><td colspan='7' align='center' class='head'>"+ hdate_get_month_string(h.gd_mon,0)+" "+hdate_get_month_string(h_ent_m.gd_mon,0)
                           + " "+year_g+"</td> "
                               "</tr>");
             ...
             for(int j= 0;j<6;j++){
                 ...
                 for (int k=7;k>0;k--){
                      ...
                      html= html + "<td height='33%' align='left'>"+QString::number(h.gd_day)+"</td>";
                            html= html +"<td align='right'>"+hdate_get_int_string(h.hd_day)+"</td></tr>";

                                 html= html +"<tr><td>&nbsp;</td></tr>";

                            html= html +"<tr><td height='33%' valign='bottom' align='center' colspan='2'>";
                            if(hol){
                                 html= html +"<span style='font-size:7pt;'>"+hdate_get_holyday_string(hol,0)+"</span>";
                                 if(reading){
                                     html= html + "<span style='font-size:7pt;'> "+hdate_get_parasha_string (reading, 1).replace("<br>"," ")+"</span>";
                                 }
                             }else{
                                 if(reading){
                                     html= html + "<span style='font-size:7pt;'>"+hdate_get_parasha_string (reading, 1).replace("<br>"," ")+"</span>";
