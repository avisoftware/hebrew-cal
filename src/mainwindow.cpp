...
void MainWindow::setTimesForSelectin(int index){
    ...
    QString dayString=get_int_to_heb_string(cw->getDay());
    QString HdayString=get_int_to_heb_string(cw->getHDay());
    QString HyearString=get_int_to_heb_string(cw->getHYear());
    ui->full_heb_date->setText(QString::fromUtf8("יום ")+dayString+" "
                              +HdayString+" "+get_hebrew_month_string(cw->getHMont(),0)
                               +" "+HyearString );
...
void MainWindow::setTimesByLibhdate(QDate theDate){
    ...
    setZmanimLabelsToEmpty();
        int sun_hour=0, first_light=0, talit=0, sunrise=0;
        int midday=0, sunset=0, first_stars=0, three_stars=0;
        int first_light_mga=0,first_stars_mga=0, sun_hour_mga=0;
                int three_stars_czhish=0;
                int place_holder;
        ...
        hdate_get_utc_sun_time_deg (theDate.day(), theDate.month(), theDate.year(), latitude, longitude, 99.28, &place_holder, &three_stars_czhish);
        /* first light and first stars by the magen avraham*/
        hdate_get_utc_sun_time_deg (theDate.day(), theDate.month(), theDate.year(), latitude, longitude,109.75, &first_light_mga, &first_stars_mga);
...
int MainWindow::printHeader(){
    QString str;
    //heb
    str = get_hebrew_month_string(monthStartHeb,0);
    if(monthStartHeb!=monthEndHeb){
        str= str+ " " + get_hebrew_month_string(monthEndHeb,0);
    }
    str = str+" "+ get_int_to_heb_string(yearStartHeb);
    if (yearStartHeb!=yearEndHeb){
        str = str+" "+ get_int_to_heb_string(yearEndHeb);
    }
    //gre
    str = str+ " - "+get_month_string(monthStartG,0);
    if (monthStartG!=monthEndG){
        str = str+ " "+get_month_string(monthEndG,0);
    }
